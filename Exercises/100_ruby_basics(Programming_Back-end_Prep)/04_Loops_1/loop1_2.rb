...
# 
# We begin by modifying the innermost block. This loop can be stopped by placing break on the line following #puts. 
# This forces the loop to iterate only once. After we've fixed the innermost loop, our attention is now focused on the parent loop. 
# We modify this loop the same way we modified the child loop: by placing break on the line following the end of the innermost loop.
# 
# The code in this exercise is considered bad practice. 
# It's used to illustrate how to break out of a nested loop, not to encourage the use of nested loops.