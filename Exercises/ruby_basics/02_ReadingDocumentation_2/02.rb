...
# 
# The part to the left of the → shows that insert takes a required index argument, and 0 or more obj arguments. We know that index is required because the name is shown in the signature with no ornamentation; such standalone names in the arguments portion of a signature are required arguments. 
# Similarly, we know that we need 0 or more obj arguments 
# because this time the name is adorned by ..., which should always be interpreted in the documentation as "0 or more occurrences of the item to the left".
# 
# 
...
# 
# These two statements produce identical results; this shows that both a and the return value have the same values. 
# In fact, they are the same object, though it needs a bit more work to show that:
# 
# b = a.insert(3, 5, 6, 7)
...
# http://ruby-doc.org/core-2.4.0/Array.html#method-i-insert